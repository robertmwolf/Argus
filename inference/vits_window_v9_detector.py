"""ViT-S/16 window_v9 heatmap detector (asl_cldice loss).

Uses the ``VITS_V9_*`` env-var namespace.

Environment variables
---------------------
VITS_V9_HEATMAP_CHECKPOINT
    Default: ``weights/vits_v9_asl_cldice/best.pt``
VITS_V9_HEATMAP_THRESHOLD
    Heatmap binarisation threshold (float, default 0.70).
VITS_V9_HEATMAP_MIN_PIXELS
    Minimum component size in feature-map pixels (int, default 2).
VITS_V9_HEATMAP_NATIVE_TILE_SIZE
    Native-pixel tile size (int, default 400).
VITS_V9_HEATMAP_TILE_OVERLAP
    Fractional overlap between adjacent tiles (float, default 0.5).
VITS_V9_HEATMAP_PEAK_FLOOR
    Minimum peak heatmap value for a detection to survive (float, default 0.85).
VITS_V9_HEATMAP_NORM
...
_DEFAULT_CHECKPOINT = (
    Path(__file__).resolve().parent.parent / "weights" / "vits_v9_asl_cldice" / "best.pt"
)


def _default_checkpoint() -> Path:
    return Path(os.environ.get("VITS_V9_HEATMAP_CHECKPOINT", str(_DEFAULT_CHECKPOINT)))


def get_vits_v9_heatmap_status() -> dict[str, str]:
...
def run_vits_v9_heatmap_detector(
    array: np.ndarray,
    checkpoint: Path | None = None,
) -> list[dict[str, Any]]:
    ckpt_path = Path(checkpoint) if checkpoint else _default_checkpoint()
    threshold        = float(os.environ.get("VITS_V9_HEATMAP_THRESHOLD", "0.70"))
    min_pixels       = int(os.environ.get("VITS_V9_HEATMAP_MIN_PIXELS", "2"))
    native_tile_size = int(os.environ.get("VITS_V9_HEATMAP_NATIVE_TILE_SIZE", "400"))
    tile_overlap     = float(os.environ.get("VITS_V9_HEATMAP_TILE_OVERLAP", "0.5"))
    peak_floor       = float(os.environ.get("VITS_V9_HEATMAP_PEAK_FLOOR", "0.85"))
    top_k            = int(os.environ.get("VITS_V9_HEATMAP_TOPK", "0"))

    try:
        model, image_size, device, use_geometry = _load_model(ckpt_path)
    except Exception as exc:
        logger.warning("ViT-S v9 heatmap model load failed: %s", exc)
        return []
    ...
    logger.debug("ViT-S v9 heatmap (tiled): %d raw → %d after NMS + peak/top-K",
                 len(all_dets), len(result))
    return result


def run_vits_v9_heatmap_detector_and_heatmap(
    array: np.ndarray,
    checkpoint: Path | None = None,
    ...
    try:
        ...
        logger.warning("ViT-S v9 heatmap model load failed (heatmap): %s", exc)
        return [], None
