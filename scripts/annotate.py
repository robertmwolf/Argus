...
Usage:
    ...
    # Review only the 50 worst-endpoint-error images flagged by analyze_endpoint_errors.py:
    python scripts/annotate.py \\
        --image-dir /Volumes/External/TrainingData/raw/BrentImages \\
        --pending-from $ARGUS_DATA_ROOT/annotations/val_balanced_v1.json \\
        --output $ARGUS_DATA_ROOT/annotations/val_balanced_v1.json
...
# ---- image list loading ------------------------------------------------------
def load_pending_direct(pending_paths: set[pathlib.Path]) -> list[dict]:
    """Build an image list directly from a set of known paths.

    Used when --pending-from is supplied so we don't need to rglob a large
    directory tree and can still reach files spread across multiple roots
    (e.g. BrentImages tiles + synthetic_eval).
    """
    found, missing = [], []
    for p in sorted(pending_paths):
        if p.is_file():
            found.append({"path": p, "score": 0.0})
        else:
            missing.append(p)
    if missing:
        log.warning(
            "%d pending files not found on disk (skipping):\n  %s",
            len(missing), "\n  ".join(str(p) for p in missing[:10]),
        )
    if not found:
        raise FileNotFoundError("None of the pending annotation files exist on disk")
    return found


def load_from_dir(image_dir: pathlib.Path, priority_list: pathlib.Path | None = None,
                  min_score: float = 0.0) -> list[dict]:
    """Load all images from a directory tree (FITS, PNG, or JPEG).

    Searches recursively into subdirectories.
    """
...
class AnnotationApp(tk.Tk):
    ...
    def _load_image(self, idx: int) -> None:
        ...
        try:
            raw = load_image(entry["path"])
        except Exception as exc:
            log.error("Cannot open %s: %s", entry["path"], exc)
            raw = Image.new("RGB", (1200, 800), (30, 30, 50))

        # If the COCO image entry has a tile_origin, the FITS file is the full
        # frame and the annotation coordinates are tile-local.  Crop to the
        # tile so OBBs render in the right place.
        self._tile_origin: tuple[int, int] | None = None
        fname = str(entry["path"])
        coco_img = next(
            (img for img in self.coco["images"] if img["file_name"] == fname), None
        )
        if coco_img:
            to = coco_img.get("tile_origin")
            tw = coco_img.get("width")
            th = coco_img.get("height")
            if to and tw and th and (raw.width > tw or raw.height > th):
                x0, y0 = int(to[0]), int(to[1])
                self._tile_origin = (x0, y0)
                raw = raw.crop((x0, y0, x0 + tw, y0 + th))
                log.debug("Cropped to tile %dx%d at origin (%d,%d)", tw, th, x0, y0)
        ...
        # Normalise OBBs to tile-local space.  Some annotations were saved in
        # full-frame coordinates; subtract tile_origin so they render correctly
        # on the cropped tile.
        def _to_tile_local(obb: dict) -> dict:
            if self._tile_origin is None:
                return obb
            ox, oy = self._tile_origin
            cx, cy = obb.get("cx", 0), obb.get("cy", 0)
            # If cx/cy are outside the tile bounds they're in full-frame space.
            tw2 = coco_img["width"] if coco_img else self._pil_img.width
            th2 = coco_img["height"] if coco_img else self._pil_img.height
            if cx > tw2 or cy > th2:
                obb = dict(obb)
                obb["cx"] = cx - ox
                obb["cy"] = cy - oy
        ...
        self._img_obbs = [_to_tile_local(ann["obb"]) for ann in anns]
        self._img_obbs_needs_review = [
...
def main() -> None:
    ...
    parser.add_argument(
        "--pending-from", type=pathlib.Path, default=None,
        metavar="COCO_JSON",
        help="COCO annotation JSON produced by analyze_endpoint_errors.py --patch-annotations. "
             "When given with --image-dir, only images whose annotation has "
             "review_status=pending in this file are shown.",
    )
    args = parser.parse_args()

    if not args.pending_from and bool(args.image_dir) == bool(args.night_dir):
        parser.error("Provide exactly one of --image-dir or --night-dir (or use --pending-from alone)")

    strk_frames: list[dict] = []

    if args.pending_from and not args.pending_from.exists():
        parser.error(f"--pending-from file not found: {args.pending_from}")
    if args.pending_from:
        with open(args.pending_from) as fh:
            pending_coco = json.load(fh)
        id_to_fname = {img["id"]: img["file_name"] for img in pending_coco.get("images", [])}
        pending_ids = {
            ann["image_id"]
            for ann in pending_coco.get("annotations", [])
            if ann.get("review_status") == "pending"
        }
        # Resolve relative file_names against cwd so synthetic paths work
        pending_paths: set[pathlib.Path] = {
            pathlib.Path(id_to_fname[iid]).resolve()
            for iid in pending_ids
            if iid in id_to_fname
        }
        log.info("--pending-from: %d annotations to review", len(pending_paths))
        try:
            images = load_pending_direct(pending_paths)
            log.info("Loaded %d pending images", len(images))
    ...
    source_name = args.source_name or (
        args.night_dir.name if args.night_dir
        else args.image_dir.name if args.image_dir
        else "pending_review"
    )
    ...
    # Defer the first image load until after the event loop starts so the
    # canvas has its real dimensions before _redraw() runs.
    app.after(0, lambda: app._load_image(start_idx))
    app.mainloop()
