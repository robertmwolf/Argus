#!/usr/bin/env bash
# Train + eval vits_taper8_asl_cldice: ViT-S asl_cldice with endpoint taper.
#
...
#
# Usage:
#   bash scripts/train_taper8_pipeline.sh 2>&1 | tee /tmp/taper8_$(date +%Y%m%d_%H%M%S).log
...
VITS_CACHE=$CACHE_ROOT/vits_taper8

TAG=vits_taper8_asl_cldice

TRAIN_ANN="$DATA/train_atwood_synth_window_v9/annotation.json"
VAL_ANN="$DATA/val_atwood_window_v9/annotation.json"

echo "=== taper8 pipeline | $(date) | vits asl_cldice + endpoint_taper_px=8 ==="
...
echo ""
echo "=== taper8 threshold sweep (val_balanced_v1, pf=0.85) ==="
$PYTHON - <<'PYEOF'
import json, pathlib
...
print()
for t in thresholds:
    p = pathlib.Path(f"results/taper8/vits_taper8_asl_cldice/pf85/geometry_eval_t{t}.json")
    if not p.exists():
        print(f"  vits_taper8  t=0.{t}  (missing)")
        continue
    d = json.loads(p.read_text())
    t1, t2, pb = d["tier1_detection"], d["tier2_raw_geometry"], d["tier1_detection"]["per_band"]
    ep_path = pathlib.Path(f"results/taper8/vits_taper8_asl_cldice/pf85/endpoint_error_analysis_t{t}.json")
    ep_mean = "n/a"
    if ep_path.exists():
        ep = json.loads(ep_path.read_text())
        ep_mean = f"{ep['symmetric_endpoint_error']['all']['mean']:>7.1f}"
    print(f"  {'vits_taper8_asl_cldice  t=0.' + t:<34}  {t1['detection_recall']:>7.3f}  {t1['detection_precision']:>7.3f}"
          f"  {pb['short']['recall']:>6.3f}  {pb['medium']['recall']:>6.3f}  {pb['long']['recall']:>6.3f}"
          f"  {t2['angle_err_deg']['mean']:>6.2f}  {ep_mean}")
PYEOF

echo ""
echo "=== taper8 pipeline complete | $(date) ==="
