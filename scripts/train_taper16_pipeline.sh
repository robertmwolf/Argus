#!/usr/bin/env bash
# Train + eval vits_taper16_asl_cldice: ViT-S asl_cldice with endpoint taper.
#
...
#
# Usage:
#   bash scripts/train_taper16_pipeline.sh 2>&1 | tee /tmp/taper16_$(date +%Y%m%d_%H%M%S).log
...
VITS_CACHE=$CACHE_ROOT/vits_taper16

TAG=vits_taper16_asl_cldice

TRAIN_ANN="$DATA/train_atwood_synth_window_v9/annotation.json"
VAL_ANN="$DATA/val_atwood_window_v9/annotation.json"

echo "=== taper16 pipeline | $(date) | vits asl_cldice + endpoint_taper_px=16 ==="
...
# ── Step 5: Endpoint error analysis (key metric for this experiment) ──────────
...
echo ""
echo "=== taper16 vs v9 asl_cldice baseline (val_balanced_v1, t=0.70, pf=0.85) ==="
$PYTHON - <<'PYEOF'
import json, pathlib

variants = [
    ("vits_v9_asl_cldice (baseline)", "results/window_v9/vits_v9_asl_cldice/pf85/geometry_eval.json"),
    ("vits_taper16_asl_cldice      ", "results/taper16/vits_taper16_asl_cldice/pf85/geometry_eval.json"),
]
...
echo ""
echo "=== taper16 pipeline complete | $(date) ==="
