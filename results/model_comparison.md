...
Numbers are reported on the **test split** (`test.json`) unless otherwise noted in
the Status column.  mAP values are COCO bbox_mAP metrics from MMDetection's evaluator.
...
| mAP@0.5 | maximise | **0.740** (DINOv3 ViT-B Phase C², test.json) — clean retrain result pending |
